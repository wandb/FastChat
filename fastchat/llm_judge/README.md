...
#### Step 1. Generate model answers to MT-bench questions
```
python gen_model_answer.py --bench-name korean_mt_bench --model-path [MODEL-PATH] --model-id [MODEL-ID]
```
Arguments:
...
```
python gen_judgment.py --bench-name korean_mt_bench --model-list [LIST-OF-MODEL-ID] --parallel [num-concurrent-api-call]
```

e.g.,
```
python gen_judgment.py --bench-name korean_mt_bench --model-list vicuna-13b-v1.3 alpaca-13b llama-13b claude-v1 gpt-3.5-turbo gpt-4 --parallel 2
```
The judgments will be saved to `data/korean_mt_bench/model_judgment/gpt-4_single.jsonl`

#### Step 3. Show MT-bench scores

- Show the scores for selected models
  ```
  python show_result.py --bench-name korean_mt_bench --model-list vicuna-13b-v1.3 alpaca-13b llama-13b claude-v1 gpt-3.5-turbo gpt-4
  ```
- Show all scores
  ```
  python show_result.py --bench-name korean_mt_bench
...
```
python gen_judgment.py --bench-name korean_mt_bench --mode pairwise-baseline --model-list vicuna-13b-v1.3 alpaca-13b llama-13b --parallel 2
```
The judgments will be saved to `data/korean_mt_bench/model_judgment/gpt-4_pair.jsonl`
...
```
python gen_judgment.py --bench-name korean_mt_bench --mode pairwise-all --model-list [LIST-OF-MODEL-ID] --parallel [num-concurrent-api-call]
```

```
python show_result.py --bench-name korean_mt_bench --mode pairwise-all
```

### How to get GPT-3.5/GPT-4/Claude's answer?
- `python gen_api_answer.py --bench-name korean_mt_bench --model [MODEL-NAME]` to generate GPT-3.5/4 and Claude's answers.
...
<img src="data/korean_mt_bench/misc/radar.png" width="600" height="450">
